...
class AutoCategorizer {
  ...
  static final List<_Rule> _rules = [
    ...
    _Rule(
      RegExp(r'^FC\d{2}-\d{4}-\d+-\d+.*(STOCK|\bIB\b|SHARESIES|HATCH|'
          r'INVESTNOW|KERNEL)'),
      const AutoCategory('Investments', 'Investments'),
    ),
    _Rule(
      RegExp(r'\bIRD\b|INLAND REVENUE|\bACC\b|\bMSD\b|WORK AND INCOME|'
          r'STUDYLINK|KIWISAVER'),
      const AutoCategory('National government services', 'Professional Services'),
    ),
    _Rule(
      RegExp(r'WOOLWORTHS|NEW WORLD|PAK.?N.?SAVE|COUNTDOWN|FRESH CHOICE|'
          r'FOUR SQUARE|SUPERETTE|\bALDI\b|\bCOSTCO\b|FARRO|MOORE WILSON|'
          r'\bIGA\b'),
      const AutoCategory('Supermarkets and grocery stores', 'Food'),
    ),
    _Rule(
      RegExp(r"MCDONALD|KFC|BURGER KING|SUBWAY|DOMINO|PIZZA HUT|WENDY'?S|"
    ...
    _Rule(
      RegExp(r'\bUBER\b|\bOLA\b|\bLYFT\b|\bBOLT\b|ZOOMY|\bTAXI\b|\bCABS?|'
    ...
    _Rule(
      RegExp(r'\bBP\b|Z ENERGY|\bMOBIL\b|CALTEX|\bGULL\b|CHEVRON|'
          r'ALLIED PETROLEUM|WAITOMO FUEL|NPD FUEL'),
      const AutoCategory('Fuel stations', 'Transport'),
    ),
    _Rule(
      RegExp(r'AIR ?NZ|JETSTAR|\bQANTAS\b|VIRGIN AUSTRALIA|EMIRATES|'
          r'\bAIRLINE\b|\bAIRWAYS\b'),
      const AutoCategory('Airlines', 'Transport'),
    ),
    _Rule(
      RegExp(r'\bAIRBNB\b|BOOKING\.COM|HOTELS\.COM|\bEXPEDIA\b|\bMOTEL\b|'
          r'\bHOTEL\b|HOLIDAY PARK|BACKPACKERS'),
      const AutoCategory('Hotels, motels, temporary accommodation', 'Lifestyle'),
    ),
    _Rule(
      RegExp(r'\bGYM\b|FITNESS|ALLFIT|LES MILLS|ANYTIME FIT|EZYPAY|'
          r'\bF45\b|SNAP FITNESS|\bJETTS\b|CITY FITNESS|CROSSFIT|'
          r'PLANET FITNESS'),
      const AutoCategory('Gym and fitness services', 'Health'),
    ),
    _Rule(
      RegExp(r'\bVULTR\b|\bAWS\b|AMAZON WEB|DIGITALOCEAN|\bLINODE\b|'
      ...
      const AutoCategory('Business software and cloud services', 'Professional Services'),
    ),
    _Rule(
      RegExp(r'NETFLIX|SPOTIFY|DISNEY|APPLE\.COM/BILL|PRIME VIDEO|'
      ...
      const AutoCategory('Entertainment (not elsewhere classified)', 'Lifestyle'),
    ),
    _Rule(
      RegExp(r'PB TECH|NOEL LEEMING|JB HI-?FI|HARVEY NORMAN|MIGHTY APE|'
          r'COOLMOBILE|MYGUY|DICK SMITH|\bKOGAN\b'),
      const AutoCategory('Electronic and appliance stores', 'Household'),
    ),
    _Rule(
      RegExp(r'AMAZON|REDBUBBLE|\bEBAY\b|\bTEMU\b|ALIEXPRESS|TRADE ME|'
    ...
    _Rule(
      RegExp(r'POWERSHOP|MERCURY|CONTACT ENERGY|GENESIS|TRUSTPOWER|'
          r'ELECTRIC KIWI|\bFLICK\b|NOVA ENERGY|PULSE ENERGY|'
          r'ENERGYONLINE'),
      const AutoCategory('Electricity services', 'Utilities'),
    ),
    _Rule(
      RegExp(r'2DEGREES|SPARK|VODAFONE|ONE ?NZ|SKINNY|SLINGSHOT|\bORCON\b|'
          r'\bVOYAGER\b|\bFYX\b'),
    ...
    _Rule(
      RegExp(r'APPLIANCE|SERVICING|HANDYMAN|\bPLUMBER\b|ELECTRICIAN|'
    ...
    _Rule(
      ...
      const AutoCategory('Accounting and tax services', 'Professional Services'),
    ),
    _Rule(
      RegExp(r'\bINSURANCE\b|\bAMI\b|\bTOWER\b|STATE INSURANCE|\bVERO\b|'
          r'\bFMG\b|FIDELITY LIFE|AA INSURANCE'),
      const AutoCategory('Insurance', 'Insurance'),
    ),
    _Rule(
      RegExp(r'NZ ?POST|COURIERPOST|\bARAMEX\b|\bDHL\b|\bFEDEX\b|'
          r'\bCOURIER\b|PARCEL'),
      const AutoCategory('Postal and courier services', 'Household'),
    ),
    _Rule(
      RegExp(r'UNIVERSITY|POLYTECHNIC|\bPOLYTECH\b|SCHOOL FEES|'
          r'\bTUITION\b|\bWANANGA\b'),
      const AutoCategory('Education services', 'Education'),
    ),
    _Rule(
      RegExp(r'\bFEE\b|OFFSHORESERVICEMARGIN|\bINTEREST\b|OVERDRAFT|'
          r'\bSURCHARGE\b'),
      const AutoCategory('Fees', 'Fees'),
    ),
    _Rule(
      RegExp(r'\bRENT\b|\bBOND\b'),
      const AutoCategory('Rent', 'Housing'),
    ),
