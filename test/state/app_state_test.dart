...
void main() {
  ...
  group('aggregations', () {
    test(
      'aggMonthly buckets income and expense into the current month',
      () async {
        final now = DateTime.now();
        final thisMonth = DateTime(
          now.year,
    ...
    test('aggCategories sums spend per group, largest first', () async {
      final state = await seededState(
        transactions: [bpFuel(), uberTrip(), mcdonaldsBurger()],
      );
    ...
    test(
      'aggWeekly groups spend within the trailing window by week start',
      () async {
        final state = await seededState(
          transactions: [bpFuel(), uberTrip(), mcdonaldsBurger()],
        );
        ...
        expect(
          state.aggWeekly.fold(0.0, (sum, w) => sum + w.total),
          closeTo(87.20 + 23.50 + 14.90, 0.001),
        );
      },
    );
  });
