...
void main() {
  ...
  group('accounts', () {
    test(
      'saveAccounts writes real account data that watchAccounts streams back',
      () async {
    ...
    test(
      'saving an account twice with the same id updates rather than duplicates',
      () async {
  ...
  group('transactions', () {
    test(
      'saveTransactions writes real merchant data that watchTransactions streams back',
      () async {
    ...
    test('watchTransactions filters to a single account by id', () async {
      ...
      final asbRows = await db
          .watchTransactions(accountId: 'acc_asb_streamline')
          .first;
  ...
  group('category overrides', () {
    test(
      'saveCategoryOverride then loadCategoryOverrides returns the real category name',
      () async {
    ...
    test(
      'saving a second override for the same transaction replaces the first',
      () async {
        final tx = mcdonaldsBurger();
