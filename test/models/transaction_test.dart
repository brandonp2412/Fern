...
void main() {
  group('Transaction.fromJson / toJson', () {
    test("McDonald's transaction round-trips through JSON", () {
      ...
      final roundTripped = Transaction.fromJson(json.decode(json.encode(tx.toJson())));
      expect(roundTripped.merchant?.name, "McDonald's");
      expect(roundTripped.amount, -14.90);
      expect(roundTripped.account, 'acc_anz_everyday');
    });

    test('a payday DIRECT CREDIT transaction has no merchant, title falls back to description', () {
  ...
  group('TransactionCategory.groupName', () {
    test('a Netflix transaction categorised under "Entertainment" reports that group', () {
      ...
      final withCategory = Transaction.fromJson({
        ...
        'category': {
          ...
          'groups': {
            'personal_finance': {'_id': 'grp_entertainment', 'name': 'Entertainment'},
          },
        },
      });
    ...
    test('falls back to the first available group when personal_finance is absent', () {
      final tx = Transaction.fromJson({
